...
function mapStatus(s: string): string {
  ...
  if (['PG','PO','DR','AP','PR','FL','SP','PS','SO','DP'].includes(code)) return 'active';
  if (['SI','TA'].includes(code)) return 'shut-in';
  return 'inactive'; // WP=Well Plugged, WD=Well Dry, NO=New Operator(?), ND, PL
}
...
// WY basins by lat/lon
function getBasin(lat: number, lon: number): string {
  if (lat > 44 && lon > -108) return 'Bighorn Basin';
  if (lat > 43 && lon < -109.5) return 'Green River Basin';
  if (lat < 42.5 && lon < -108) return 'Green River Basin';
  if (lat < 43 && lon > -107) return 'Powder River Basin';
  if (lat > 43 && lat < 45 && lon > -107) return 'Powder River Basin';
  if (lat > 41 && lat < 43 && lon > -107.5 && lon < -105) return 'Wind River Basin';
  return 'Powder River Basin'; // default - largest WY basin
}

async function main() {
  console.log('WY Well Ingestion (FracTracker National Wells Part2)\n');

  let allWells: any[] = [];
  let offset = 0;
  while (true) {
    const params = new URLSearchParams({
      where: "State='WY'", outFields: 'OBJECTID,API,Name,Operator,Status,Type,Lat,Long',
  ...
  try {
    ...
    for (const [key, name] of opMap) {
      await client.query(`INSERT INTO operators (id, legal_name, hq_state, active_asset_count) VALUES ($1,$2,'WY',0) ON CONFLICT (id) DO UPDATE SET legal_name=EXCLUDED.legal_name`,
        [dUUID('wy_operator', key), name]);
    }
    ...
    for (let i = 0; i < allWells.length; i += CHUNK) {
      ...
      for (const w of chunk) {
        ...
        const id = dUUID('wy_well', key);
        if (seen.has(id)) { skipped++; continue; }
        seen.add(id);

        const basin = getBasin(w.Lat || 0, w.Long || 0);
        const opKey = w.Operator ? w.Operator.trim().toUpperCase() : null;
        const opId = opKey ? dUUID('wy_operator', opKey) : null;
        const status = mapStatus(w.Status || '');
        const type = (w.Type || '').toUpperCase() === 'G' || (w.Status || '').toUpperCase() === 'PG' ? 'gas' : 'oil';
        const name = w.Name ? titleCase(w.Name) : `WY Well ${w.OBJECTID}`;

        vals.push(`($${pi},$${pi+1},$${pi+2},$${pi+3},$${pi+4},$${pi+5},$${pi+6},$${pi+7},$${pi+8},$${pi+9})`);
        params.push(id, type, name, 'WY', '', w.Lat||0, w.Long||0, basin, opId, status);
        pi += 10;
      }
    ...
    await client.query(`UPDATE operators SET active_asset_count=sub.cnt FROM (SELECT operator_id,count(*)::int AS cnt FROM assets WHERE state='WY' AND status='active' GROUP BY operator_id) sub WHERE operators.id=sub.operator_id`);

    await client.query(`INSERT INTO data_provenance (id,source_name,source_url,record_count,status,notes) VALUES ($1,'WY_FRACTRACKER',$2,$3,'success',$4)`,
      [randomUUID(), BASE_URL, inserted, `${inserted} WY wells from FracTracker National Wells Part2`]);

    const { rows: [s] } = await client.query(`SELECT count(*) AS t, count(*) FILTER (WHERE status='active') AS a, count(*) FILTER (WHERE status='shut-in') AS si FROM assets WHERE state='WY'`);
    console.log(`\nWY: ${s.t} total, ${s.a} active, ${s.si} shut-in`);
