...
function mapStatus(s: string): string {
  ...
  if (lower.includes('active') || lower.includes('produc')) return 'active';
  if (lower.includes('shut') || lower.includes('inactive')) return 'shut-in';
  return 'inactive';
}
...
// PA basins - Appalachian Basin dominates, with Marcellus/Utica being the big unconventional plays
function getBasin(lat: number, lon: number): string {
  // Southwest PA (Washington, Greene counties) = core Marcellus
  // Northeast PA (Susquehanna, Bradford) = also Marcellus
  // All of PA oil/gas is Appalachian Basin
  return 'Appalachian Basin';
}

async function main() {
  console.log('PA Well Ingestion (FracTracker National Wells Part2)\n');

  let allWells: any[] = [];
  let offset = 0;
  while (true) {
    const params = new URLSearchParams({
      where: "State='PA'", outFields: 'OBJECTID,API,Name,Operator,Status,Type,Lat,Long',
  ...
  try {
    ...
    for (const [key, name] of opMap) {
      await client.query(`INSERT INTO operators (id, legal_name, hq_state, active_asset_count) VALUES ($1,$2,'PA',0) ON CONFLICT (id) DO UPDATE SET legal_name=EXCLUDED.legal_name`,
        [dUUID('pa_operator', key), name]);
    }
    ...
    for (let i = 0; i < allWells.length; i += CHUNK) {
      ...
      for (const w of chunk) {
        // No API numbers - use OBJECTID + name as key
        const key = w.API || `${w.OBJECTID}-${w.Name || ''}-${w.Lat}-${w.Long}`;
        const id = dUUID('pa_well', key);
        if (seen.has(id)) { skipped++; continue; }
        seen.add(id);

        const basin = getBasin(w.Lat || 0, w.Long || 0);
        const opKey = w.Operator ? w.Operator.trim().toUpperCase() : null;
        const opId = opKey ? dUUID('pa_operator', opKey) : null;
        const status = mapStatus(w.Status || '');
        const type = (w.Type || '').toLowerCase().includes('gas') ? 'gas' : 'oil';
        const name = w.Name ? titleCase(w.Name) : `PA Well ${w.OBJECTID}`;

        vals.push(`($${pi},$${pi+1},$${pi+2},$${pi+3},$${pi+4},$${pi+5},$${pi+6},$${pi+7},$${pi+8},$${pi+9})`);
        params.push(id, type, name, 'PA', '', w.Lat||0, w.Long||0, basin, opId, status);
        pi += 10;
      }
    ...
    await client.query(`UPDATE operators SET active_asset_count=sub.cnt FROM (SELECT operator_id,count(*)::int AS cnt FROM assets WHERE state='PA' AND status='active' GROUP BY operator_id) sub WHERE operators.id=sub.operator_id`);

    await client.query(`INSERT INTO data_provenance (id,source_name,source_url,record_count,status,notes) VALUES ($1,'PA_FRACTRACKER',$2,$3,'success',$4)`,
      [randomUUID(), BASE_URL, inserted, `${inserted} PA wells from FracTracker National Wells Part2`]);

    const { rows: [s] } = await client.query(`SELECT count(*) AS t, count(*) FILTER (WHERE status='active') AS a, count(*) FILTER (WHERE status='shut-in') AS si FROM assets WHERE state='PA'`);
    console.log(`\nPA: ${s.t} total, ${s.a} active, ${s.si} shut-in`);
