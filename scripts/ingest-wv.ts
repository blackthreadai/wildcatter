...
function mapStatus(s: string): string {
  ...
  if (lower.includes('active') || lower.includes('produc') || lower.includes('permit issued') || lower.includes('future')) return 'active';
  if (lower.includes('shut')) return 'shut-in';
  return 'inactive';
}
...
async function main() {
  console.log('WV Well Ingestion (FracTracker National Wells Part2)\n');

  let allWells: any[] = [];
  let offset = 0;
  while (true) {
    const params = new URLSearchParams({
      where: "State='WV'", outFields: 'OBJECTID,API,Name,Operator,Status,Type,Lat,Long',
  ...
  try {
    if (opMap.size > 0) {
      ...
      for (const [key, name] of opMap) {
        await client.query(`INSERT INTO operators (id, legal_name, hq_state, active_asset_count) VALUES ($1,$2,'WV',0) ON CONFLICT (id) DO UPDATE SET legal_name=EXCLUDED.legal_name`,
          [dUUID('wv_operator', key), name]);
      }
    }
    ...
    if (opMap.size > 0) {
      await client.query(`UPDATE operators SET active_asset_count=sub.cnt FROM (SELECT operator_id,count(*)::int AS cnt FROM assets WHERE state='WV' AND status='active' GROUP BY operator_id) sub WHERE operators.id=sub.operator_id`);
    }

    await client.query(`INSERT INTO data_provenance (id,source_name,source_url,record_count,status,notes) VALUES ($1,'WV_FRACTRACKER',$2,$3,'success',$4)`,
      [randomUUID(), BASE_URL, inserted, `${inserted} WV wells from FracTracker National Wells Part2`]);

    const { rows: [s] } = await client.query(`SELECT count(*) AS t, count(*) FILTER (WHERE status='active') AS a, count(*) FILTER (WHERE status='shut-in') AS si FROM assets WHERE state='WV'`);
    console.log(`\nWV: ${s.t} total, ${s.a} active, ${s.si} shut-in`);
