...
async function main() {
  console.log('OH Well Ingestion (FracTracker National Wells Part2)\n');

  let allWells: any[] = [];
  let offset = 0;
  while (true) {
    const params = new URLSearchParams({
      where: "State='OH'", outFields: 'OBJECTID,API,Name,Operator,Status,Type,Lat,Long',
  ...
  try {
    ...
    for (const [key, name] of opMap) {
      await client.query(`INSERT INTO operators (id, legal_name, hq_state, active_asset_count) VALUES ($1,$2,'OH',0) ON CONFLICT (id) DO UPDATE SET legal_name=EXCLUDED.legal_name`,
        [dUUID('oh_operator', key), name]);
    }
    ...
    for (let i = 0; i < allWells.length; i += CHUNK) {
      ...
      for (const w of chunk) {
        ...
        const id = dUUID('oh_well', key);
        if (seen.has(id)) { skipped++; continue; }
        seen.add(id);
        const opKey = w.Operator ? w.Operator.trim().toUpperCase() : null;
        const opId = opKey ? dUUID('oh_operator', opKey) : null;
        // All OH statuses are None — default to active (this dataset is primarily active/permitted wells)
        const status = 'active';
        const type = (w.Type||'').toLowerCase().includes('gas') && !(w.Type||'').toLowerCase().includes('oil') ? 'gas' : 'oil';
        const name = w.Name ? titleCase(w.Name) : `OH Well ${w.OBJECTID}`;
        vals.push(`($${pi},$${pi+1},$${pi+2},$${pi+3},$${pi+4},$${pi+5},$${pi+6},$${pi+7},$${pi+8},$${pi+9})`);
        params.push(id, type, name, 'OH', '', w.Lat||0, w.Long||0, 'Appalachian Basin', opId, status);
        pi += 10;
      }
      if (vals.length > 0) {
        await client.query(`INSERT INTO assets (id,asset_type,name,state,county,latitude,longitude,basin,operator_id,status) VALUES ${vals.join(',')} ON CONFLICT (id) DO UPDATE SET name=EXCLUDED.name,operator_id=EXCLUDED.operator_id`, params);
    ...
    await client.query(`UPDATE operators SET active_asset_count=sub.cnt FROM (SELECT operator_id,count(*)::int AS cnt FROM assets WHERE state='OH' AND status='active' GROUP BY operator_id) sub WHERE operators.id=sub.operator_id`);
    await client.query(`INSERT INTO data_provenance (id,source_name,source_url,record_count,status,notes) VALUES ($1,'OH_FRACTRACKER',$2,$3,'success',$4)`, [randomUUID(), BASE_URL, inserted, `${inserted} OH wells`]);

    const { rows: [s] } = await client.query(`SELECT count(*) AS t, count(*) FILTER (WHERE status='active') AS a FROM assets WHERE state='OH'`);
    console.log(`\nOH: ${s.t} total, ${s.a} active`);
