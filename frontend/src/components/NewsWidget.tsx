...
export default function NewsWidget({ region = 'US', title }: NewsWidgetProps) {
  ...
  useEffect(() => {
    ...
    const refreshInterval = (region === 'GLOBAL' || region === 'US' || region === 'ASIAN' || region === 'SOUTH AMERICAN' || region === 'RUSSIAN' || region === 'MIDDLE EAST ENERGY' || region === 'EUROPEAN ENERGY' || region === 'AFRICAN') ? 15 * 60 * 1000 : 35 * 60 * 1000; // 15 min for global/energy regions, 35 min for others
  ...
  return (
    <div className="w-full flex flex-col bg-black border border-gray-700 h-full">
      ...
      <div className="flex-1 p-2 pb-8 overflow-y-auto bg-black min-h-0">
        {articles.length === 0 ? (
