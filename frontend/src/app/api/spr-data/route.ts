...
async function fetchEIASPRData(): Promise<SPRResponse | null> {
  try {
    ...
    const url = `https://api.eia.gov/v2/petroleum/stoc/wstk/data/?api_key=${eiaApiKey}&frequency=weekly&data[0]=value&facets[series][]=WCSSTUS1&sort[0][column]=period&sort[0][direction]=desc&length=300`;
    ...
    for (let i = 0; i < rawData.length; i += 4) { // Sample every 4th week (monthly)
      ...
      if (point && point.period && point.value) {
        ...
        const valueInMillions = Math.round(parseFloat(point.value) / 1000);
        historical.push({
          date: point.period,
    ...
    // Convert current level from thousand barrels to million barrels
    const currentLevel = rawData[0]?.value ? Math.round(parseFloat(rawData[0].value) / 1000) : 0;
