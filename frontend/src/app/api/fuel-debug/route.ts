...
export async function GET() {
  ...
  const descs = latest.map(r => ({ desc: r['series-description'], value: r.value, product: r.product }));
  return NextResponse.json({ period: rows[0]?.period, total: descs.length, all: descs });
}
